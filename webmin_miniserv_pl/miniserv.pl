...
if (!$full || $is_directory) {
	...
	ROOT: foreach $root (@roots) {
		...
		foreach $b ($simple eq "/" ? ( "" ) : split(/\//, $simple)) {
			...
			if ($sofar eq "/restapi") {
				$is_restapi = 1;
				print DEBUG "restapi found: $sofar\n";
				next;
				}
			#
			@stfull = stat($full);
	...
	if (!@stfull && $is_restapi == 0) { &http_error(404, "File not found"); }
	if ($is_restapi == 0 && $method eq "PUT" ) {
		print DEBUG " 400 $is_restapi $method\n";
		&http_error(400, "Bad Request method $method");
		}
	}
...
# chinchen
if ($is_restapi) {
	print DEBUG "handle restapi Request URI: $resturi, method: $method\n";
	if($header{'content-type'} eq 'application/json') {
		if($method ne "GET" && !$posted_data) {
			$clen = $header{"content-length"};
			$clen_read = $clen > $config{'max_post'} ? $config{'max_post'} : $clen;
			while(length($posted_data) < $clen_read) {
				$buf = &read_data($clen_read - length($posted_data));
		...
		local $resp = "HTTP/1.0 $ok_code $ok_message\r\n".
    	    ...
    	    &http_error(406, "Not Acceptable");
   		}
}
elsif (&get_type($full) eq "internal/cgi" && $validated != 4) {
