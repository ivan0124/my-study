...
#endif
static int mqtt_client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[]);
...
int mqtt_client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	int rc;
	FILE *fptr;
	char line[1024];
	...
	/* Default config file */
#ifndef WIN32
	//env = getenv("MQTT_CONFIG_HOME");
	//env = ".";
	env = MQTT_CONFIG_DIR;
	if(env){
		len = strlen(env) + strlen("/mqtt_pub.conf") + 1;
		loc = malloc(len);
		if(pub_or_sub == CLIENT_PUB){
			snprintf(loc, len, "%s/mqtt_pub.conf", env);
		}else{
			snprintf(loc, len, "%s/mqtt_sub.conf", env);
		}
		loc[len-1] = '\0';
	}else{
		env = getenv("HOME");
		if(env){
			len = strlen(env) + strlen("/.config/mqtt_pub.conf") + 1;
			loc = malloc(len);
			if(pub_or_sub == CLIENT_PUB){
				snprintf(loc, len, "%s/.config/mqtt_pub.conf", env);
			}else{
				snprintf(loc, len, "%s/.config/mqtt_sub.conf", env);
			}
			loc[len-1] = '\0';
		}else{
	...
	printf("Load MQTT Sub config: %s\n", loc);

#else
	rc = 1;//GetEnvironmentVariable("USERPROFILE", env, 1024);
	sprintf(env, "%s", MQTT_CONFIG_DIR);
	if(rc > 0 && rc < 1024){
		len = strlen(env) + strlen("\\mqtt_pub.conf") + 1;
		loc = malloc(len);
		if(pub_or_sub == CLIENT_PUB){
		...
		if(fptr){
			while(fgets(line, 1024, fptr)){
				...
				if(args[1]){
					...
					rc = mqtt_client_config_line_proc(cfg, pub_or_sub, count, args);
					if(rc){
						fclose(fptr);
						free(loc);
						return rc;
	...
	rc = mqtt_client_config_line_proc(cfg, pub_or_sub, argc, argv);
	if(rc) return rc;
...
int mqtt_client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	int i;
	...
	for(i=1; i<argc; i++){
		if(!strcmp(argv[i], "-p") || !strcmp(argv[i], "--port")){
			...
			//printf("%s: Connect to MQTT host: %s\n", __func__, cfg->host);
			i++;
#ifdef WITH_TLS
			...
			if(i==argc-1){
				...
				if(pub_or_sub == CLIENT_PUB){
					...
					//printf("%s: Sub Topic: %s\n", __func__, cfg->topics[cfg->topic_count-1]);
				}
				i++;
			}
